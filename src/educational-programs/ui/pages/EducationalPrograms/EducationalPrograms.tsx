...
import {
  useEducationalProgramsExecute,
  useEducationalProgramsQuery,
} from '@/educational-programs/hooks';
import { useMemo, useState } from 'react';
import { EducationalProgramModal, AcademicDegree, StudyFormat } from '@/educational-programs/ui/components';
...
const EducationalPrograms = () => {
  ...
  const { executeRequest: deleteEducationalPrograms } = useEducationalProgramsExecute<GrouppedEPData[]>({ serviceFunction: "deleteEducationalPrograms" });

  const [selectedStudyFields, setSelectedStudyFields] = useState<GrouppedEPData[]>([]);
  const [selectedSpecialties, setSelectedSpecialties] = useState<GrouppedEPData[]>([]);
  const [selectedEducationalPrograms, setSelectedEducationalPrograms] = useState<GrouppedEPData[]>([]);
  ...
  const handleStudyFieldsSelected = (data: Array<EducationalProgram["studyField"]>) => {
    setSelectedStudyFields(data.map(item => ({
      data1: item,
    })));
  }

  const handleSpecialtiesSelected = (studyField: EducationalProgram["studyField"], data: Array<EducationalProgram["specialty"]>) => {
    setSelectedSpecialties(data.map(item => ({
      data1: studyField,
      data2: item,
    })));
  };

  const handleEducationalProgramsSelected = (studyField: EducationalProgram["studyField"], specialty: EducationalProgram["specialty"], data: Array<EducationalProgram["educationalProgram"]>) => {
    setSelectedEducationalPrograms(data.map(item => ({
      data1: studyField,
      data2: specialty,
      data3: item,
    })));
  ...
  return (
    <Root>
      ...
      <EducationalProgramModal
        open={!!modalVisible}
        educationalProgram={selectedItems.length ? { ...selectedItems[0] } : { data1: {} } as GrouppedEPData}
        mode={modalVisible}

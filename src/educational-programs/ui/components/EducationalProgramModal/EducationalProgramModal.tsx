...
import {
  useAddEducationProgram,
  useEditEducationProgram,
} from '@/educational-programs/hooks/index.ts';
...
const EducationalProgramModal = ({
  ...
  const { executeRequest: editEducationalProgram, isPending: isEditPending } =
    useEditEducationProgram({
      onSuccess: onClose,
    });

  const { executeRequest: addEducationalProgram, isPending: isAddPending } =
    useAddEducationProgram({ onSuccess: onClose });

  const { register, handleSubmit, formState, control, reset } =
    useForm<EducationalProgram>({
      mode: 'onChange',
  ...
  const modalInfo = useMemo(() => {
    if (
      (modalMode == 'edit' && values?.educationalProgram) ||
    ...
    if (
      (modalMode == 'edit' && values?.specialty) ||
    ...
    if (
      (modalMode == 'edit' && values?.studyField) ||
    ...
    <Modal open={open} onClose={onClose}>
      <Content>
        ...
        <Form onSubmit={handleSubmit(onSubmit)}>
          ...
          {modalInfo?.type === 'educationalProgram' && (
          ...
          <Actions>
            ...
            <SaveButton
              loading={isEditPending || isAddPending}
              disabled={!isValid}
              type="submit"
            />
          </Actions>
